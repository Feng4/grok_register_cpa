...
def cloudflare_build_headers(content_type=False):
    ...
    if key:
        ...
        elif mode in ("x-admin-auth", "admin"):
            # cloudflare_temp_email admin API: POST /admin/new_address
...
def cloudflare_create_temp_address(api_base):
    """创建临时邮箱地址，返回 (address, jwt)。

    兼容 cloudflare_temp_email:
    - 公开/用户侧: POST /api/new_address
    - 管理端: POST /admin/new_address + header x-admin-auth

    路径读 cloudflare_path_accounts（默认 /api/new_address），
    鉴权走 cloudflare_auth_mode / cloudflare_api_key。
    """
    ...
    try:
        ...
        pass
    # admin 接口文档要求 name / enablePrefix；公开接口多忽略多余字段
    if "admin" in path:
        payload.setdefault("enablePrefix", True)
        if not payload.get("name"):
            chars = string.ascii_lowercase + string.digits
            payload["name"] = "".join(secrets.choice(chars) for _ in range(10))
...
def cloudflare_create_account(api_base, address, password, api_key=None, expires_in=0):
    ...
    if api_key and "X-API-Key" in headers:
        headers["X-API-Key"] = api_key
    if api_key and get_cloudflare_auth_mode() in ("x-admin-auth", "admin"):
        headers["x-admin-auth"] = api_key
    payload = {"address": address, "password": password, "expiresIn": expires_in}
    # path_accounts 在 temp-email 模式下常配为 /admin/new_address 或 /api/new_address；
    # Mail.tm 风格建号不能复用该路径，回退 /accounts。
    path = get_cloudflare_path("cloudflare_path_accounts", "/accounts")
    if "new_address" in path:
        path = "/accounts"
    params = cloudflare_apply_auth_params()
...
# 管理员 JWT 单例（用于创建邮箱账户）
_cloudmail_admin_token = None
_cloudmail_admin_token_lock = threading.Lock()


def _cloudmail_get_admin_token(force_refresh=False):
    """获取或刷新共享的管理员 JWT（线程安全单例）"""
    global _cloudmail_admin_token
    with _cloudmail_admin_token_lock:
        if _cloudmail_admin_token and not force_refresh:
            return _cloudmail_admin_token
        url = get_cloudmail_url()
        admin_email = get_cloudmail_admin_email()
        admin_password = get_cloudmail_password()
        if not url or not admin_email or not admin_password:
        ...
        if not jwt:
            raise Exception("CloudMail 管理员登录失败")
        _cloudmail_admin_token = jwt
        return jwt


def cloudmail_ensure_account(email, name=""):
    """用管理员账号在 CloudMail 创建邮箱账户。

    CloudMail 若未配置 Cloudflare Email Routing catch-all，必须先创建
    account 该地址才有收信路由，否则发往该地址的邮件会被 555 退信。
    使用 admin 的 /api/account/add（无需验证码），区别于 /api/register。
    """
    url = get_cloudmail_url()
    try:
        jwt = _cloudmail_get_admin_token()
    except Exception as exc:
        raise Exception(f"CloudMail 管理员登录失败: {exc}")
    payload = {"email": email, "name": name or email.split("@")[0]}
    ...
    if isinstance(data, dict) and data.get("code") == 200:
        return data.get("data", {})
    msg = str(data.get("message", "")) if isinstance(data, dict) else str(data)[:200]
    # 已存在视为成功（并发或重试可能命中）
    if any(k in msg.lower() for k in ("exist", "已存在", "duplicate")):
        return {}
    # token 失效则刷新一次重试
    if any(k in msg.lower() for k in ("token", "401", "无权", "登录", "auth")):
        try:
            jwt = _cloudmail_get_admin_token(force_refresh=True)
            resp = _add(jwt)
            resp.raise_for_status()
            data = resp.json()
            if isinstance(data, dict) and data.get("code") == 200:
            ...
            pass
    raise Exception(f"CloudMail 创建邮箱失败: {msg}")
...
def get_email_and_token(api_key=None):
    ...
    if provider == "cloudmail":
        ...
        # 必须先创建 account，否则该地址无收信路由，验证码邮件会被 555 退信
        cloudmail_ensure_account(address, username)
...
class GrokRegisterGUI:
    ...
    def setup_ui(self):
        ...
        self.cloudflare_auth_mode_combo = ttk.Combobox(
            ...
            values=["x-admin-auth", "query-key", "bearer", "x-api-key", "none"],
            width=12,
            state="readonly",
        )
...
- 必须与后端真实路由一致
- 官方 admin 推荐（诊断修复后默认）:
  - /api/domains,/admin/new_address,/api/token,/api/mails
- 公开创建备选:
...
- 官方 admin: 鉴权模式选 x-admin-auth，key 填 Worker 管理密码（请求头 x-admin-auth）
- needAuth=false 且公开创建: 模式 none，path 用 /api/new_address
- 其他: bearer / x-api-key / query-key
- create_temp_address 会读取 path_accounts + auth_mode + api_key，勿再硬编码
...
- 如果关闭了启动教程，可随时点主界面的“教程”按钮重新打开。"""
    ...
    def _run_single_registration(self, idx, total, logf):
        ...
        # ── CPA: 用注册浏览器页面做设备码授权 ──
        if config.get("cpa_export_enabled", True) and sso:
            try:
                import cpa_export
                _page = _get_page()  # 复用已登录的注册浏览器
                result = cpa_export.export_cpa_xai_for_account(
                    email, profile.get("password", ""),
                    page=_page, sso=sso, config=config,
                    log_callback=lambda m: logf(f"[CPA] {m}"),
                )
                if result.get("ok"):
                    logf(f"[CPA] + {result.get('path')}")
                else:
                    logf(f"[CPA] ! {result.get('error', 'unknown')}")
