...
> **授权码流程（Authorization Code + PKCE）** 换 OIDC token，写成 CPA 的 `type=xai` 认证文件。  
> authorize 和 consent 两处注入 `referrer=grok-build`，确保 token 能被 `cli-chat-proxy.grok.com` 接受。

本仓库**自包含** OIDC/CPA 铸造代码（已内置 `cpa_xai/`）：

- 库：`./cpa_xai/`
- **授权码流程（默认）**：`./cpa_xai/oauth_authcode.py` — 纯 HTTP，无需浏览器，~2-5s
- 成功 hook：`./cpa_export.py` — 自动检测 SSO，走授权码流程
- 批量补认证：`./scripts/backfill_cpa_xai_from_accounts.py` — 同样默认授权码
- Device flow（旧，回退用）：`./cpa_xai/oauth_device.py` + `browser_confirm.py`
...
[邮箱 CloudMail] → 注册 accounts.x.ai
 ...
 grok2api 池 (可选)        SSO → Web 非 4.5 模型
       ↓
 授权码流程 (Auth Code+PKCE) 纯 HTTP，无浏览器，~2-5s
   ├─ GET /oauth2/authorize?referrer=grok-build&plan=generic
   ├─ POST consent (referrer=grok-build)
   └─ POST /oauth2/token → access_token (referrer=grok-build)
       ↓
 校验 referrer claim       JWT payload 必须含 referrer=grok-build
       ↓
...
| `cpa_force_device_flow` | 强制旧版 device + 浏览器流程 | 默认 `false`，自动走授权码流程 |
...
| 现象 | 原因 / 处理 |
|------|-------------|
| 授权码流程：SSO 无效 | SSO cookie 过期或格式错误；重新注册或从 `accounts_cli.txt` 取最新 |
| 授权码流程：consent 不返回 code | SSO 已过期或 xAI 风控；检查代理是否正常 |
| 授权码流程：token 无 referrer claim | 极罕见（代码已硬编码注入）；检查日志 |
| 有 token 但无 grok-4.5 | `base_url` 是否为 `cli-chat-proxy`；probe 失败勿导入 |
| CPA 401 | api-key 错；或 auth 未进热加载目录 |
| 注册成功但无 `cpa_auths` | `cpa_export_enabled`？看日志 / `cpa_auth_failed.txt` |
| 回退到 device flow | SSO 未传入或设了 `cpa_force_device_flow=true`；日志会标注 flow= |

### device flow 遗留问题（仅 `cpa_force_device_flow=true` 时相关）
