...
def create_address(
    ...
    """创建临时邮箱。path 可为 /api/new_address 或 /admin/new_address。"""
...
def main():
    ...
    ap.add_argument("--path", default="/api/new_address", help="create path: /api/new_address or /admin/new_address")
    ap.add_argument("--api-key", default="", help="auth key for create (admin password when using x-admin-auth)")
    ap.add_argument("--auth-mode", default="none", help="none|bearer|x-api-key|query-key|x-admin-auth")
    ap.add_argument("--domain", default="", help="optional domain for create payload")
